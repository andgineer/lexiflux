...
def test_ai_settings_api_post_valid_data(client, approved_user):
    ...
    for model in SUPPORTED_CHAT_MODELS:
        ...
        test_data.append({"chat_model": model, "settings": settings})

    response = client.post(
        reverse("ai_settings_api"), data=json.dumps(test_data), content_type="application/json"
    )
...
def test_ai_settings_api_post_invalid_model(client, approved_user):
    ...
    test_data = [{"chat_model": "InvalidModel", "settings": {"key": "value"}}]
...
def test_ai_settings_api_invalid_json(client, approved_user):
    ...
    response = client.post(
        reverse("ai_settings_api"), data="invalid json", content_type="application/json"
    )
