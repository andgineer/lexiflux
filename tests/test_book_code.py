...
def test_generate_unique_book_code_cyrillic():
    ...
    book = Book(title="Война и мир", author=author, language=language)
    book.save()
...
def test_generate_unique_book_code_chinese():
    ...
    book = Book(title="道德经", author=author, language=language)
    book.save()
...
def test_generate_unique_book_code_mixed_script():
    ...
    book = Book(title="1Q84 いちきゅうはちよん", author=author, language=language)
    book.save()
...
def test_generate_unique_book_code_with_diacritics():
    ...
    book = Book(title="Gargantua et Pantagruel", author=author, language=language)
    book.save()
...
def test_generate_unique_book_code_arabic():
    ...
    book = Book(title="أولاد حارتنا", author=author, language=language)
    book.save()
...
def test_generate_unique_book_code_greek():
    ...
    book = Book(title="Βίος και Πολιτεία του Αλέξη Ζορμπά", author=author, language=language)
    book.save()
...
def test_generate_unique_book_code_collision():
    ...
    book1 = Book(title="The Test Book", author=author1, language=language)
    book1.save()

    book2 = Book(title="The Test Book", author=author2, language=language)
    book2.save()
...
def test_generate_unique_book_code_special_characters():
    ...
    language = Language.objects.get(name="English")
    book = Book(title="S*p(e)c_i+a=l C_h/a\\r<a>c{t}e[r]s", author=author, language=language)
    book.save()
...
def test_generate_unique_book_code_empty():
    ...
    language = Language.objects.get(name="English")
    book = Book(title="*()_+= _/\\<>{}[]", author=author, language=language)
    book.save()
