...
from lexiflux.language.llm import Llm, AIModelError, _remove_word_marks, _remove_sentence_marks, _extract_sentence, \
    find_nth_occurrence, safe_float, TextOutputParser
...
class TestTermDetection:
    @pytest.mark.parametrize("text,term,occurrence,expected", [
        ("The cat sat on the mat. The cat ran.", "cat", 1, {"term_word_ids": [1],
                                                            "word_slices": [(0, 3), (4, 7), (8, 11), (12, 14), (15, 18),
                                                                            (19, 22), (24, 27), (28, 31), (32, 35)]}),
        ("The cat sat on the mat. The cat ran.", "cat", 2, {"term_word_ids": [7],
                                                            "word_slices": [(0, 3), (4, 7), (8, 11), (12, 14), (15, 18),
                                                                            (19, 22), (24, 27), (28, 31), (32, 35)]}),
    ])
...
class TestModelSettings:
    def test_get_model_settings_with_user_config(self, db_init, approved_user):
        ...
        with patch('lexiflux.models.AIModelConfig.get_or_create_ai_model_config', return_value=config):
    ...
    def test_get_model_settings_fallback_to_env(self, db_init, approved_user):
        ...
        with patch('lexiflux.models.AIModelConfig.get_or_create_ai_model_config', return_value=config), \
             patch('lexiflux.language.llm.settings', create=True, new_callable=MagicMock) as mock_settings:
...
class TestTextProcessing:
    ...
    @pytest.mark.parametrize("text, expected", [
        ...
        (f"{SENTENCE_START_MARK}test{SENTENCE_END_MARK}", "test"),
        (f"{WORD_START_MARK}word{WORD_END_MARK} in {SENTENCE_START_MARK}sentence{SENTENCE_END_MARK}", "sentence"),
    ])
...
class TestUtilityFunctions:
    @pytest.mark.parametrize("text, substring, occurrence, expected", [
