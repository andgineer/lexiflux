...
def test_library_view_books_list_for_regular_user(client, user, book):
    ...
    another_user = get_user_model().objects.create_user(
        "another_user", password="12345", email="email"
    )
    shared_book = Book.objects.create(
        title="Shared Book",
        author=book.author,
        language=book.language,
        code="shared-book-code",
        owner=another_user,
        public=False,
    )
...
def test_library_view_for_superuser(client, book):
    ...
    another_user = get_user_model().objects.create_user(
        "another_user", "another@example.com", "password123"
    )
    ...
    Book.objects.create(
        ...
        author=author,
        language=language,
        code="another-book-code",
        owner=another_user,
        public=False,
    )
    Book.objects.create(
        title="Public Book", author=author, language=language, code="public-book-code", public=True
    )
...
def test_library_view_pagination(client, user, author, book):
    ...
    for i in range(
        2, total_books + 1
    ):  # Starting from 2 because one book is already created by the fixture
        Book.objects.create(
            title=f"Book {i}",
            author=author,
            language=language,
            ...
            owner=user
            if i % 2 == 0
            else None,  # Some owned by the user, some not to vary the setup
...
class TestImportBook:
    ...
    def test_import_book_success_simple_file(
        self, client, approved_user, book, file_ext, loader_class
    ):
        client.force_login(approved_user)
    ...
    def test_import_book_with_temporary_file(self, client, approved_user, book):
        ...
        with patch("lexiflux.views.library_views.BookLoaderPlainText") as MockLoader:
            ...
            with patch(
                "django.core.files.uploadhandler.TemporaryFileUploadHandler.new_file",
                return_value=mock_temp_file,
            ):
                response = client.post(
                    reverse("import_book"), {"file": SimpleUploadedFile("test.txt", b"content")}
                )
...
class TestBookDetailView:
    ...
    def test_update_book_not_found(self, client, approved_user):
        ...
        response = client.post(
            ...
            data={"title": "New Title", "author": "New Author", "language": "en"},
        )
        assert response.status_code == 400
        assert f"Book ({{'id': {invalid_id}}}) not found" in str(response.json()["error"])
    ...
    def test_update_book_success(self, client, approved_user, book, language):
        ...
        response = client.post(
            reverse("edit_book_modal", kwargs={"book_id": book.id}), data=new_data
        )
    ...
    def test_delete_book_unauthorized(self, client, book):
        ...
        assert f"{other_user.email} does not have permission to read book '(ID: {book.id})" in str(
            response.json()["error"]
        )
    ...
    def test_update_book_invalid_language(self, client, approved_user, book):
        ...
        response = client.post(
            reverse("edit_book_modal", kwargs={"book_id": book.id}),
            data={"title": "Updated Title", "author": "New Author", "language": "invalid_code"},
        )
        assert response.status_code == 400
