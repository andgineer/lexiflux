...
def test_set_sources_replaces_image_sources(book, client):
    ...
    expected_url = reverse('serve_book_image', kwargs={
        'book_code': book.code,
        'image_filename': test_image.filename
    })
...
def test_serve_book_image_success(client, book, approved_user):
    ...
    response = client.get(reverse('serve_book_image', kwargs={
        'book_code': book.code,
        'image_filename': test_image.filename
    }))
...
def test_link_click_success(client, approved_user, book):
    ...
    book.anchor_map = {
        "#chapter1": {"page": 2, "word": 5}
    }
    book.save()

    response = client.post(reverse('link_click'), {
        'book-code': book.code,
        'link': '#chapter1'
    })
