...
### Wiktionary
Переводчику Wiktionary нужны данные, которых нет в образе. Импортируйте их в контейнер (около
3,2 ГБ скачивания, несколько минут, на диске остаётся около 230 МБ):

    docker exec -it lexiflux ./manage import-wiktionary

Смотрите [Данные Wiktionary](aimodels.md#wiktionary-data). Словарь хранится в контейнере,
поэтому попадает в [резервную копию](docker.md#backup).
